...
type
  ...
  private
    ...
    procedure SaveMeasurementSettings(var Ms: TMeasurementSettings; Init: Boolean);
...
// CRLF seperated list to semicolon list
    ...
    SaveAllSettings(SETTINGTYPE, __Config, MeasurementSettings, cbLoggerIP, AppInit);
  end;
end;
...
// allow events to write settings
  AppInit := False;
end;
...
// ID the page by name not index, avoids problems after adding or removing pages
  ...
  if not AppInit then
  begin
    SaveMeasurementSettings(MeasurementSettings, AppInit);
    SaveAllSettings(SETTINGTYPE, __Config, MeasurementSettings, cbLoggerIP, AppInit);
  end;
end;
...
// stop events from writing setting during initialisation
...
//  -----------------------
  ...
  tbSampleWindow.Position := MeasurementSettings.WindowSize;
...
begin
(* The sample window is 3 to 23 so (tbSampleWindow.Position * 2) + 3 *)
...
procedure TMainForm.SaveMeasurementSettings(var Ms: TMeasurementSettings; Init: Boolean);
begin
  if not Init then
  begin
